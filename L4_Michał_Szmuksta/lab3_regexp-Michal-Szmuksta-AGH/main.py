...
def film_in_category(category:Union[int,str])->pd.DataFrame:
    ...
    if isinstance(category, int):
        db = pd.read_sql("SELECT title, la.name, category FROM film WHERE film_id = {} ORDER BY title, category".format(category), con=connection)
        return db
    elif isinstance(category, str):
        db = pd.read_sql("SELECT title, language, category FROM film WHERE title = '{}' ORDER BY title, category".format(category), con=connection)
        return db
    else:
        return None
